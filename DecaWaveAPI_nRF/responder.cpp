...
#define POLL_RX_TO_RESP_TX_DLY_UUS 10000//2600
/* This is the delay from the end of the frame transmission to the enable of the receiver, as programmed for the DW1000's wait for response feature. */
#define RESP_TX_TO_FINAL_RX_DLY_UUS 500
/* Receive final timeout. See NOTE 5 below. */
#define FINAL_RX_TIMEOUT_UUS 12000//3300
...
int main(void)
{
    ...
    {
        pc.printf("DW initialise failed\r\n");
        return 1;
    }
    pc.printf("DW initalise succeeds\r\n");

    /* Configure DW1000. See NOTE 3 below. */
    dwt_configure(&config);
    pc.printf("DW configure succeeds\r\n");
    ...
    while (1)
    {
        ...
        if (status_reg & SYS_STATUS_RXFCG)
        {
            ...
            if (memcmp(rx_buffer, rx_poll_msg, ALL_MSG_COMMON_LEN) == 0)
            {
                ...
                //pc.printf("status 2: %x\r\n", dwt_read32bitreg(SYS_STATUS_ID));
                /* Poll for reception of expected "final" frame or error/timeout. See NOTE 8 below. */
                while (!((status_reg = dwt_read32bitreg(SYS_STATUS_ID)) & (SYS_STATUS_RXFCG | SYS_STATUS_ALL_RX_TO | SYS_STATUS_ALL_RX_ERR)))
                { };
                pc.printf("status 3: %x\r\n", status_reg);
                /* Increment frame sequence number after transmission of the response message (modulo 256). */
                frame_seq_nb++;

                if (status_reg & SYS_STATUS_RXFCG)
                {
                    pc.printf("Final frame\r\n");
